-- ============================================================================
-- Top 5 European Football Leagues Database Schema - SUPABASE VERSION
-- Database: Supabase (PostgreSQL)
-- Author: Senior Database Engineer (20+ years experience)
-- Version: 2.0 - Supabase Compatible
-- Description: Enterprise-grade PostgreSQL schema adapted for Supabase
--              All psql meta-commands removed (\c, \echo, DROP/CREATE DATABASE)
-- ============================================================================

-- NOTE: In Supabase, you cannot DROP/CREATE databases or use \c commands
-- This script creates the schema in your existing Supabase project database

-- ============================================================================
-- Enable extensions for better performance and functionality
-- ============================================================================

-- Note: Some extensions may require Supabase admin approval
-- If you get permission errors, contact Supabase support or use available extensions
...
-- Drop existing types if they exist (for re-runnable script)
-- WARNING: CASCADE will drop dependent tables/columns that use these types
-- If you have existing data, consider backing up first or use a migration approach
-- Order matters: drop dependent types first
DROP TYPE IF EXISTS shot_situation_type CASCADE;
DROP TYPE IF EXISTS shot_result_type CASCADE;
...
COMMENT ON COLUMN leagues.updated_at IS 'Last modification timestamp';
...
CREATE INDEX IF NOT EXISTS idx_teams_league ON teams(league_id) WHERE is_active = true;
...
CREATE INDEX IF NOT EXISTS idx_players_name_pattern ON players(name varchar_pattern_ops);
...
CREATE INDEX IF NOT EXISTS idx_team_players_current ON team_players(player_id, team_id) WHERE is_current = true;
CREATE INDEX IF NOT EXISTS idx_team_players_season ON team_players(season_start, season_end);
...
-- Improvement: Added status, updated_at
...
CREATE INDEX IF NOT EXISTS idx_games_status ON games(status) WHERE status != 'completed';
CREATE INDEX IF NOT EXISTS idx_games_teams_date ON games(home_team_id, away_team_id, date);
CREATE INDEX IF NOT EXISTS idx_games_recent ON games(date DESC, league_id) WHERE status = 'completed';
...
--              Separated composite key into single PK with UNIQUE constraint
CREATE TABLE IF NOT EXISTS team_stats (
    ...
    -- Unique constraint: prevent duplicate team stats for same game
    CONSTRAINT uq_team_stats_game_team UNIQUE (game_id, team_id)
);

COMMENT ON TABLE team_stats IS 'Detailed team statistics for each game';
COMMENT ON COLUMN team_stats.team_stat_id IS 'Auto-generated primary key';
...
CREATE INDEX IF NOT EXISTS idx_team_stats_result ON team_stats(result);
CREATE INDEX IF NOT EXISTS idx_team_stats_team_covering ON team_stats(team_id, goals, x_goals, shots, result);
...
CREATE TABLE IF NOT EXISTS appearances (
    ...
    -- Unique constraint: prevent duplicate appearances for same player in same game
    CONSTRAINT uq_appearances_game_player UNIQUE (game_id, player_id)
);

COMMENT ON TABLE appearances IS 'Player performance statistics for each game';
COMMENT ON COLUMN appearances.appearance_id IS 'Auto-generated primary key';
...
CREATE INDEX IF NOT EXISTS idx_appearances_position ON appearances(position) WHERE position IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_appearances_player_stats ON appearances(
    player_id, goals, assists, time_played
);
CREATE INDEX IF NOT EXISTS idx_appearances_goals ON appearances(player_id, goals) WHERE goals > 0;
CREATE INDEX IF NOT EXISTS idx_appearances_assists ON appearances(player_id, assists) WHERE assists > 0;
...
CREATE INDEX IF NOT EXISTS idx_shots_assister ON shots(assister_id) WHERE assister_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_shots_result ON shots(shot_result) WHERE shot_result IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_shots_situation ON shots(situation) WHERE situation IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_shots_game_shooter ON shots(game_id, shooter_id);
CREATE INDEX IF NOT EXISTS idx_shots_shooter_result ON shots(shooter_id, shot_result, x_goal);
CREATE INDEX IF NOT EXISTS idx_shots_goals ON shots(game_id, shooter_id, minute) 
    WHERE shot_result = 'Goal';

-- GiST index for spatial queries (shot positions)
CREATE INDEX IF NOT EXISTS idx_shots_position ON shots USING gist(
...
DROP TRIGGER IF EXISTS trg_team_stats_updated_at ON team_stats;
CREATE TRIGGER trg_team_stats_updated_at
    BEFORE UPDATE ON team_stats
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_timestamp();

DROP TRIGGER IF EXISTS trg_appearances_updated_at ON appearances;
...
DROP TRIGGER IF EXISTS trg_team_stats_validate ON team_stats;
...
DROP TRIGGER IF EXISTS trg_appearances_validate_team ON appearances;
...
DROP TRIGGER IF EXISTS trg_shots_validate_team ON shots;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_league_standings_unique ON mv_league_standings(league_id, season, team_id);
CREATE INDEX IF NOT EXISTS idx_mv_league_standings_league_season ON mv_league_standings(league_id, season, points DESC);
CREATE INDEX IF NOT EXISTS idx_mv_league_standings_team ON mv_league_standings(team_id, season);
